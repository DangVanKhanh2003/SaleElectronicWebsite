...

{
    ...
    public class CustomerAccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> ChangePassword(ChangePasswordModel model)
        {
            ...
            {
                _uow.CreateTransaction();
                var check = await _uow.CustomersAccount.ChangePassword(model.email, model.password, model.newPassword);
                await _uow.Save();
                _uow.Commit();
        ...
        [HttpPut("ChangePassword2")]

        public async Task<IActionResult> ChangePassword2(ChangePasswordModel2 model)
        {
            try
            {
                _uow.CreateTransaction();
                var check = await _uow.CustomersAccount.ChangePasswordNgu(model.email, model.newPassword);
                await _uow.Save();
                _uow.Commit();
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteAccountByIdCustomer(Guid idAccount)
        {
            try
            {
                _uow.CreateTransaction();
                var result = await _uow.CustomersAccount.DeleteAccountByIdCustomer(idAccount);
                await _uow.Save();
                _uow.Commit();
