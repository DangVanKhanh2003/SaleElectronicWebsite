...
namespace SellingElectronicWebsite.Repository
{
    ...
    public class CustomerAccountRepository : ICustomerAccountRepository
    {
        ...
        public async Task<bool> DeleteAccountByIdCustomer(Guid id)
        {
            var acc = await _context.AccountCustomers.Where(p => p.AccCustomerId == id).FirstOrDefaultAsync();
            if (acc == null)
            {
                return false;
            }
        ...
        // nao can phai xoa di
        public async Task<bool> ChangePasswordNgu([FromBody] string email, [FromBody] string newPassword)
        {
            var account = await _context.AccountCustomers.Where(x => x.Email == email).FirstOrDefaultAsync();
            if (account != null)
            {
